...
wss.on("connection", (socket) => {
  socket.on("message", (data) => {
    ...
    if (parseData.type === "join") {
      ...
      if (!BOARDS[boardId]) {
        BOARDS[boardId] = [{ userId: parseData.userId, socket: socket }];
      }

      // if boardId exists, check if userId already exists in the array
      const userExists = BOARDS[boardId].some(
        (user) => user.userId === parseData.userId,
      );

      // if userId does not exist, add the user to the array
      if (!userExists) {
        BOARDS[boardId].push({ userId: parseData.userId, socket: socket });
      }
      ...
      BOARDS[boardId]
        .filter((user) => user.userId !== parseData.userId)
        .forEach(({ socket }) => {
          socket.send(
            JSON.stringify({ type: "join", userId: parseData.userId }),
          );
        });
